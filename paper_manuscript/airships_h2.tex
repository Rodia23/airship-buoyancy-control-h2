...
%=================================================================
\abstract{Heavy-lift airships develop a positive-buoyancy surplus upon payload release that, if unmanaged, drives an uncontrolled ascent. This paper derives actuator sizing laws for a closed regenerative cycle that neutralizes this surplus: H\textsubscript{2} is combusted from the ballonets to reduce net lift; the condensed water is retained as inertial ballast and recovered by on-board solar electrolysis. Three specifications are obtained in closed form and evaluated across four representative platforms spanning $7.1$--$284$~t: the minimum combustion rate, which obeys $\dotmmin \propto M_\text{total}^{-1/2}$---a \emph{scaling paradox} in which larger craft demand lower specific flow rates; the minimum condenser capacity $Q_\text{cond,min} \approx 1{,}512$~kW for a 10-min condensation window; and the minimax-optimal installed rate $\dotmstar$, resolved via a cubic equation. Substituting atmospheric air for pure oxygen reduces the descent-phase H\textsubscript{2} budget by ${\approx}36\%$ relative to onboard O\textsubscript{2} (${\approx}44.7$ vs.\ ${\approx}70$~kg/tonne). For the representative platform, anticipatory H\textsubscript{2} extraction of up to ${\approx}4$~s before each release lowers $\dotmstar$ to $5.21$~kg/s---a $21\%$ reduction below the purely-reactive analytical minimum of $6.6$~kg/s, and an $83\%$ reduction in installed actuator capacity relative to the standard hardcoded scheme ($\dotmmax = 30$~kg/s). Furthermore, a comprehensive mass-budget analysis using 2025 aerospace benchmarks supports the theoretical hardware feasibility of the proposed cycle: the integrated system requires up to 53\% of the commercial payload capacity for large-scale platforms, and three of the four evaluated airships achieve energetic self-sufficiency under average irradiance conditions.}
...
representative platforms spanning $7.1$--$284$~t. Three specifications are obtained in closed form:
...
(Appendix~\ref{sec:appendixA}). The analysis shows that larger airships are intrinsically easier to stabilize, and that the resulting flow rates constitute directly actionable combustion subsystem specifications. A subsystem-level mass budget indicates that the integrated system is consistent with the commercial payload limits of large-scale platforms, and three of the four evaluated airships achieve energetic self-sufficiency under average irradiance conditions, establishing this framework as a simulation-based design reference for future experimental investigation.}
...
\added{Figure~\ref{fig:dlim_flujo} maps $\dotmstar$ as a function of the maximum allowed preconditioning sinking depth $D_\text{lim}$ for the four evaluated platforms. For the minimax-optimal simulation case of Section~\ref{sec:dynval}, $D_\text{lim} = 0.10$~m is adopted as a conservative operational bound: it satisfies $D_\text{lim} \ll |\Delta h|_\text{max} = 5$~m (2\% of the safety threshold) while providing sufficient preconditioning depth for meaningful compensation. At this operational depth, the analytical $\dotmmin$ predictions lie within $2$--$6\%$ of the simulated threshold for the three large-scale platforms and within $25\%$ for the Zeppelin~NT (Table~\ref{tab:minimos_actualizada}), where the high-$\varepsilon$ preconditioning benefit substantially reduces the simulated threshold below the purely-reactive analytical bound.}
...
\begin{table}[H]
\centering
\caption{Characteristics of the evaluated airships. \addedR{The Max Payload column lists the nominal commercial cargo capacity reported by each platform's reference source, used as the denominator of the mass-budget envelope of Table~\ref{tab:mass_envelope}.}}
\label{tab:dirigibles}
\renewcommand{\arraystretch}{1.2}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{} l r r r r c @{}}
\toprule
\textbf{Airship} & \makecell{$A_\text{front}$\\(m²)} & \makecell{$M_\text{str}$\\(kg)} & \makecell{$A_\text{total}$\\(m²)} & \makecell{\textbf{Max Payload}\\(kg)} & $\varepsilon^{b}$ \\
\midrule
...
\multicolumn{6}{@{}p{\linewidth}}{$^{b}$ Computed for a standard $m_\text{load} = 1{,}000$~kg single-drop release ($\varepsilon = m_\text{load}/M_\text{total}$), not the max payload shown; see Section~\ref{sec:results}.} \\
\end{tabular}
\end{table}
...
\subsubsection{Dynamic Simulation and Optimization Comparison}
\label{sec:dynval}
...
\added{The two configurations differ primarily in the installed actuator capacity and the resulting operating margin. The standard scheme ($\dotmmax=30$~kg/s, $\dot{m}_\text{prec}=-10$~kg/s, $t_\text{anticip}=2.0$~s) operates with a $6\times$ oversized actuator relative to the analytical minimum, achieving a peak displacement of $\Delta h_\text{max}\approx0.37$~m well within the precision threshold ($\leq1$~m) and recovering ${\approx}460$~kWh per delivery. The minimax-optimal scheme ($\dotmstar=5.21$~kg/s, $\dot{m}_\text{prec}=-5.21$~kg/s, $t_\text{anticip}=3.83$~s, $\alpha^*=0.215$) reduces the installed actuator capacity by $83\%$ relative to the standard scheme ($\dotmmax=30$~kg/s, which carries a ${\approx}4.5\times$ margin above the analytical minimum of $6.6$~kg/s), and by $21\%$ relative to that analytical minimum, at the cost of operating closer to the safety boundary ($\Delta h_\text{max}\approx4.2$~m within the $5$~m threshold), with a higher per-cycle electrolysis demand (${\approx}964$~kWh) recoverable through the photovoltaic regeneration cycle. In both configurations, the adaptive PD controller and the O\textsubscript{2} mass-absorption mechanism ensure full state convergence and energetic closure of the regenerative cycle.}

\paragraph{Multi-platform extension.}
...
\toprule
\textbf{Platform} & \makecell{$M_\text{total}^{a}$\\(kg)} & $\alpha^*$ & \makecell{$\dotmstar$\\(kg/s)} & \makecell{$t_\text{anticip}$\\(s)} & \makecell{$E_\text{gen}$\\(kWh)} \\
\midrule
...
\bottomrule
\multicolumn{6}{@{}p{\linewidth}}{$^{a}$ $M_\text{total} = M_\text{str} + m_{H_2} + m_\text{load}$: inertial mass at the moment of release, with $m_\text{load} = 1{,}000$~kg (simulation standard drop) and $m_{H_2}$ the onboard H\textsubscript{2} gas mass. Differs from $M_\text{str} +$ Max Payload of Table~\ref{tab:dirigibles}.} \\
\end{tabular}
\end{table}
...
\addedR{The simulations confirm that the minimax-optimal scheme is robust across the full $40\times$ range of total masses ($M_\text{total}$: 7.1--284~t, Zeppelin~NT to Hindenburg), with $\alpha^* = 0.215$ remaining invariant and the required actuator flow following the analytical scaling law $\dotmstar \propto M_\text{total}^{-1/2}$.}
...
\added{The standard-scheme simulation (${\approx}460$~kWh) is consistent with this analytical lower bound (Eq.~\eqref{eq:egen}); the minimax-optimal case (${\approx}964$~kWh) exceeds it by a factor of~${\approx}2$, as the smaller actuator requires an extended post-drop PD settling window during which H$_2$ extraction continues beyond the static compensation requirement (Section~\ref{sec:dynval}).}
...
adopting as design parameters $f_\text{pan} = 0.45$ (maximum effective area with thin-film Copper Indium Gallium Selenide (CIGS) cells on the upper curvature \cite{Gao2024}), $\eta_\text{pv} = 0.20$ (photovoltaic conversion efficiency of commercial CIGS thin-film, within the 17--22\% range of aerospace-grade modules; the thin-film amorphous baseline at 120~g/m$^2$, $\eta = 8\%$ \cite{Pande2018}, is used for panel mass estimates), and $E_{H_2,\text{el}} = 45$~kWh/kg (actual PEM electrical demand, the lower bound of the 45--60~kWh/kg range for state-of-the-art electrolysers \cite{Letsios2026, Esmaeili2023PEM}; note that $E_{H_2,\text{el}}$ already accounts for PEM efficiency losses and is independent of $\eta_\text{pv}$).
...
Photovoltaic array
 ...
 & CIGS thin-film at $\eta_\text{pv} = 0.20$ (17--22\%) closes daily budget for 3 of 4 platforms under average irradiance conditions (Section~\ref{sec:balance}); amorphous baseline at 120~g/m\textsuperscript{2}, $\eta = 8\%$ \cite{Pande2018} used for array mass estimates \\
\midrule
H\textsubscript{2} storage (COPV)
 & 28.9--104.8~kg H\textsubscript{2}/cycle at 350~bar
...
\addedR{The integrated mass budget is detailed in Appendix~\ref{app:mass_budget_full} and summarised in Table~\ref{tab:mass_envelope}. The optimistic component bound applies advanced HT-PEM at 50~kg \cite{Zhang2025PEMFC} and burst-duty generator modules at 250~kg each (GE90 combustor fraction in burst-duty regime; Appendix~\ref{app:generator_bounds}); the pessimistic bound applies air-cooled PEM at 170~kg and continuous-duty generator modules at 600~kg each. The viability column reports $\checkmark$ when the system mass remains within the platform's payload capacity and $\times$ in the scaling-limit regime where it does not.}
...
\multirow{2}{*}{LZ~129 Hindenburg} & \multirow{2}{*}{12{,}500} & 4{,}309 & 34\%  & \checkmark \\
                                  &                            & 5{,}829 & 47\%  & \checkmark \\
\midrule
\multirow{2}{*}{Flying Whales LCA60T} & \multirow{2}{*}{60{,}000} & 4{,}645 & 7.7\%  & \checkmark \\
                                     &                            & 6{,}515 & 10.9\% & \checkmark \\
\midrule
...
\addedR{At the design photovoltaic efficiency $\eta_\text{pv} = 0.20$ (CIGS thin-film, Section~\ref{sec:balance}), three of the four platforms achieve sub-24~h single-drop regeneration: Hindenburg (11.2~h), Flying Whales (11.7~h), and Pathfinder~3 (20.6~h); the Zeppelin~NT requires approximately 4.0~days (96~h), consistent with its scaling-paradox position. Panel mass estimates are based on the thin-film amorphous baseline (120~g/m\textsuperscript{2}, $\eta = 8\%$, \cite{Pande2018}); higher-efficiency alternatives (single-junction $\eta=24\%$, triple-junction $\eta=32\%$) shorten the regeneration cycle proportionally at a higher surface density, as detailed in Appendix~\ref{app:panel_bounds}.}
...
\section{Expanded Subsystem Sizing Reference}
...
The PEM electrolyser sustains $44.7$~kg H\textsubscript{2}/day at ${\approx}84$~kW continuous. Table~\ref{tab:pem_bounds} brackets the system mass across three aerospace technology bounds; the NREL industrial cost-modelling envelope of 100--420~kg frames the upper reference, well above all aerospace SOTA 2025 values.
...
\midrule
Optimistic          & $\sim$250   & GE90-class combustor subassembly in burst-duty regime: Cameretti et al.\ \cite{Cameretti2025} confirm 100\% H\textsubscript{2} combustion at 121--150~MW thermal in a GE90-class chamber (full engine mass $8{,}283$~kg; combustor sub-assembly ${\approx}$5--10\%, i.e.\ 400--800~kg at continuous duty); burst-duty operation (${\sim}$30~s/cycle, ${<}10^4$ lifetime cycles) eliminates sustained thermal-fatigue loading and active cooling duty cycles, permitting a 2--3$\times$ structural mass reduction to ${\sim}$250~kg \\
Pessimistic         & $\sim$600   & GE90 combustor subassembly at continuous-duty service: 5--10\% of full engine mass ($8{,}283$~kg) yields 400--800~kg \cite{Cameretti2025}; starting from the upper end (800~kg), the mass reduction from burst-duty operation offset by integration hardware places the pessimistic bound at ${\sim}$600~kg \\
...
Flying Whales LCA60T  & 104.8 & 1{,}803 \\
...
COPV storage                & 1{,}663 & 1{,}663 & 1{,}803 & 1{,}803 &   497 &   497 &   580 &   580 \\
...
\midrule
\textbf{Total}              & \textbf{4{,}309} & \textbf{5{,}829} & \textbf{4{,}645} & \textbf{6{,}515} & \textbf{3{,}049} & \textbf{5{,}269} & \textbf{1{,}831} & \textbf{3{,}001} \\
\bottomrule
\end{tabular}
\end{table}
